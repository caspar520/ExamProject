...
//    NSMutableDictionary *threadDict = [currentThread threadDictionary];
//    NSManagedObjectContext *threadContext = [threadDict objectForKey:kKPStoreManagedObjectContextKey];
//    DLog(@"currentThread: %@, threadContext:%@", currentThread, threadContext);
//#endif
...
#if (BABYSTORY_SERVER_LEVEL== PREVIEW_SERVER)
        {
            
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"数据库错误" message:[NSString stringWithFormat:@"domain = %@,code = %d",error.domain,error.code] delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
            [alert show];
            [alert release];
            return;
        }
...
- (void)mergeChanges:(NSNotification *)notification {
  //DLog(@"mergeChanges:@", notification);
